...
namespace FinishProject
{
    public partial class Form17 : Form
    {
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            x_cartesian.Text = Convert.ToString(length * Math.Sin(duzelt * (Math.PI / 180)) * Math.Cos(astronomical_azimuth * (Math.PI / 180)));
            y_cartesian.Text = Convert.ToString(length * Math.Sin(duzelt * (Math.PI / 180)) * Math.Sin(astronomical_azimuth * (Math.PI / 180)));
            z_cartesian.Text = Convert.ToString(length * Math.Cos(duzelt * (Math.PI / 180)));
